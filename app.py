...
if 'scan_results' in st.session_state:
    ...
    with m2:
        st.metric("Trend Leaders", st.session_state.get('trend_passed', 'N/A'))
    with m3:
        st.metric("Patterns Found", len(matches))
    with m4:
...
with c2:
    cat_help = """
    Filter by fundamental strength:
    - 💎 **Platinum:** High Growth Elite (Rev/Earn > 25%, Margin > 20%)
    - 🥇 **Gold:** Strong Performer (Rev/Earn > 15%)
    - 🥈 **Silver:** Moderate/Positive Growth & Margins
    - 🥉 **Bronze:** Weak Fundamentals or Missing Data
    """
    selected_cats = st.multiselect("Fundamental Category", ["Platinum", "Gold", "Silver", "Bronze"], default=["Platinum", "Gold", "Silver"], help=cat_help)
    st.caption("ℹ️ **Note:** scanner identifies **ALL** valid patterns first. Use this filter to refine your view (e.g., to see only Platinum setups) without re-scanning.")
